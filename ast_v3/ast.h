...
extern std::map<std::string, int> func;

extern int symbol_counter;
extern int loop_counter;
extern int if_counter;
extern int else_counter;
extern int switch_counter;
extern int case_counter;
//typedef enum { coder, codel } codeType;
extern bool coder;
extern bool left_num;
extern std::string currentf;
extern int ssp_counter;
extern int ssp_par_counter;
using namespace std;
...
public :
  ...
  void pcodegen(ostream& os) {
      ...
      if (unary_) { 
          ...
          atom_->pcodegen(os);
		  switch (op_) {
		  case 286: os << "inc" << endl; break;
		  case 287: os << "neg" << endl; break;
		  case 300: os << "not" << endl; break;
		  }
      }
      else {
          assert(left_ && right_);
		  ...
		  right_->pcodegen(os);
		  left_num = false;
		  if((op_== 286) && (left_num==false) && !coder) os << "inc ";
		  if ((op_ == 287) && (left_num==false) && !coder) os << "dec ";
		  if ((op_ >= 291) && (op_ <= 299)) left_num = true;
          left_->pcodegen(os);

		  switch (op_) {
		  case 286: if (coder) os << "add" << endl; break;
		  case 287: if (coder) os << "sub" << endl; break;
		  case 288: os << "mul" << endl; break;
		  case 289: os << "div" << endl; break;
		  
		  case 291: os << "mod" << endl; break;
		  case 292: os << "les" << endl; break;
		  case 293: os << "leq" << endl; break;
		  case 294: os << "equ" << endl; break;
		  case 295: os << "neq" << endl; break;
		  case 296: os << "grt" << endl; break;
		  case 297: os << "geq" << endl; break;
		  case 298: os << "and" << endl; break;
		  case 299: os << "or" << endl; break;
...
public:
  ...
  void pcodegen(ostream& os) {
	  if (left_num) {
		  ...
		  os << std::fixed << std::setprecision(1) << r_ << endl;
	  }
...
public :
  ...
  void print (ostream& os) {
		os<<"Node name : Case"<<endl;
		assert(stat_list_);
	  stat_list_->print(os);
  }
  void pcodegen(ostream& os) {
	  int case_temp = case_counter++;
      assert(stat_list_);
	  os << "case_" << case_temp <<"_"<< switch_counter-1 << ":" << endl;
      stat_list_->pcodegen(os);
	  os << "ujp end_switch_" << switch_counter-1 << endl;
...
public :
  ...
  void pcodegen(ostream& os) {
	  int switch_temp = switch_counter;
	  //int case_temp = case_counter;
      assert(case_);
      case_->pcodegen(os);
      if (case_list_) {
	  ...
	  os << "ujp case_"<< --case_counter <<"_"<< switch_temp-1 << endl;
...
public :
  ...
  void print (ostream& os) {
		os<<"Node name : CaseStatement"<<endl;
  ...
  void pcodegen(ostream& os) {
      ...
      exp_->pcodegen(os);
	  os << "neg" << endl;
	  os << "ixj end_switch_" << switch_temp << endl;
      case_list_->pcodegen(os);
	  case_counter = case_temp;
	  os << "end_switch_" << switch_temp << ":" << endl;
...
public :
  ...
  void print (ostream& os) {
	  os << "Node name : LoopStatement" << endl;
		assert( exp_ && stat_list_);
    exp_->print(os);
		stat_list_->print(os);
  }
  void pcodegen(ostream& os) {
	  int temp = loop_counter;
	  os << "loop_" << loop_counter++ << ":"<< endl;
      assert(exp_ && stat_list_);
      exp_->pcodegen(os);
	  os << "fjp end_loop_" << temp  << endl;
      stat_list_->pcodegen(os);
	  os << "ujp loop_" << temp  << endl;
	  os << "end_loop_" << temp << ":" << endl;
  }
...
public :
  ...
  void print (ostream& os) {
		os<<"Node name : ConditionalStatement"<<endl;
  ...
  void pcodegen(ostream& os) {
	  int if_temp = if_counter++;
	  int else_temp = else_counter++;
	  
      assert(exp_ && stat_list_if_);
      exp_->pcodegen(os);
	  if (!stat_list_else_)
			os << "fjp end_if_" << if_temp << endl;
	  else
			os << "fjp else_if_" << else_temp << endl;
      stat_list_if_->pcodegen(os);

      if (stat_list_else_) {
		  os << "ujp end_if_" << if_temp << endl;
		  os << "else_if_" << else_temp << ":" << endl;
          stat_list_else_->pcodegen(os);
      }
	  os << "end_if_" << if_temp << ":" <<  endl;
  }
...
public:
  ...
  void pcodegen(ostream& os) {
	  os << "lda " << "0 5" << endl << "ind" << endl;;
	  if (!coder) {
		  os << "ldc " << symbolTable.at(*name_) << endl;
	  }
	  else if (coder)
		  os << "ldc " << symbolTable.at(*name_) << endl << "ind" << endl;

	  //else 
		 // os << "lda " << symbolTable.at(*name_) << endl;
...
public:
  ...
  void pcodegen(ostream& os) {
	  //os << "variable " << symbol_counter << endl;
	  //ssp_counter++;
	  //os << *name_ << endl;
      assert(type_);
      type_->pcodegen(os);
	  symbolTable.insert(make_pair(*name_, symbol_counter++));
...
public :
  ...
  void pcodegen(ostream& os) {
      ...
      assert(decl_);
	  //os << "ret/stp" << endl;
	  //os << "ssp3 " << ssp_counter << endl;
      decl_->pcodegen(os);
	  ssp_counter++;
	  //os << "ssp11 " << ssp_counter << endl;
...
public :
  ...
  void pcodegen(ostream& os) {
	  //os << "ssp " << ssp_counter << endl;
	  //os << "sep " << "50" << endl;
	  //os << "ujp " << currentf << "_begin" << endl;
	  
	  //ssp_counter = 5;
	  
	  std::string temp = currentf;
	  
      if (decl_list_) {
		  os << "ssp " << ssp_counter << endl;
		  os << "sep " << "50" << endl;
		  os << "ujp " << currentf << "_begin" << endl;
		  //os << temp << "_begin1:" << endl;
		  ssp_counter = 5;
          decl_list_->pcodegen(os);
		 
      }
	  os << "ssp " << ssp_par_counter << endl;
	  os << "sep " << "50" << endl;
	  os << "ujp " << currentf << "_begin" << endl;
	  os << temp << "_begin:" << endl;
	  ssp_counter = 5;
